function theta_example_panels(dataset, resdir, theta_inx, norm_spsth, time)
%THETA_EXAMPLE_PANELS   Creates supplementary panels for theta example cells (Figs. S8).
%   THETA_EXAMPLE_PANELS(DATASET, RESDIR, THETAINDEXMATRIX, NORM_SPSTH, TIME)
...
%  2025
    ...
    elseif strcmp(dataset, 'Exp')
        ...
        y_positions = {[10; 0], [10; 0], [0; 2]};
        y_limits = {[10,60], [10,70], [0,8]};
        heatmap_spacing = {1.3, 10, 40};
    end
    
    % Define parameters
    colors3 = {"#EBDB34", "#3495EB", "#A07BE0"};  % Colors for each theta group
    alignevent = 'FixationBeginning'; % Trigger event for PSTH and rasters
    sigma = 0.02;             % Smoothing kernel for SPSTH
    dt = 0.001;               % Resolution for PSTH
    wn = [-2 6];              % Window 
    bwin = [-1.6 0];          % Baseline window
    twin = [0.2 1];           % Task window
    delay_start = 0.2;        % Start of task window
    delay_end = 1;            % End of task window
    avg_y_limits = [-3 3];    % Y limits for avg PSTHs
    avg_x_limits = [-0.5 1.5];% X limits for avg PSTHs
    
    % Categorize neurons by theta index
    high_threshold = 0.5;
    mod_threshold = 0.2;
    allIndices = 1:length(theta_inx);
    highThetaIndices = allIndices(theta_inx > high_threshold);
    middleThetaIndices = allIndices(theta_inx < high_threshold & theta_inx > mod_threshold);
    lowThetaIndices = allIndices(theta_inx < mod_threshold);
    theta_group = categorizeTheta(theta_inx, high_threshold, mod_threshold);
    ...
    for g = 1:numGroups
        groupIndices = find(theta_group == g);
    ...
    for i = 1:3
        ...
        % Average PSTH
        figure(1)
        average_plot(time, norm_spsth(cell_idx, :), colors3{i}, dataset);
        ...
        % Raster plot
        figure(2)
        scatter_plot(example_cells{i}, alignevent, sigma, 'all', wn, dataset);
    
        % PSTH plot
        figure(4)
        psth_plot(example_cells{i}, alignevent, wn, dt, sigma, 'all', ...
...
function theta_group = categorizeTheta(theta_index, high_thresh, mod_thresh)
% Categorizes neurons into theta rhythmicity groups based on thresholds.

    % Initialize group labels with zeros
    theta_group = zeros(size(theta_index));
    theta_group(theta_index > high_thresh) = 1;     % Group 1: Strongly theta rhythmic
    theta_group(theta_index <= high_thresh & theta_index > mod_thresh) = 2;     % Group 2: Moderately theta rhythmic
    theta_group(theta_index <= mod_thresh) = 3;     % Group 3: Non-theta
end


function average_plot(time, data, clr, str)
...
function [WPa, WPi] = psth_plot(example_cell, alignevent, wn, dt, sigma, ...
...
function scatter_plot(example_cell, alignevent, sigma, partition, wn, str, n)
