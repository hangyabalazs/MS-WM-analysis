function [CCR, lags, ThetaIndex] = acgmod(cellids,window,varargin)
...
%   difference between mean ACG for a +-25 ms window around the peak withi
%   lags 100 and 200 ms and the mean ACG for lags 225-275ms.
...
addParameter(prs,'issave', true,@islogical)   % control saving behavior
...
addParameter(prs,'isvisual', false,@islogical)   % control visually pleasing ACG, for final form figures
...
global DATAPATH
if isempty(g.resdir)
    g.resdir = fullfile(DATAPATH,'ACG_');  % results directory
    if ~isfolder(g.resdir)
        mkdir(g.resdir)
    end
end
fnmm = 'ACG_.mat';   % filename for saving the result matrices
...
isiViolationRate = nan(numCells,1);  % preallocate
...
for iC = 1:numCells   % loop through the cells
    ...
    if length(ncc) > limit_spikes(2)      % crop if too long to avoid out of memory
        ...
        isi = diff(ncc);  % in seconds
        isi_ms = isi * 1000;  % convert to milliseconds
        
        numViolations = sum(isi_ms < 1);  % count ISIs < 1 ms
        isiViolationRate(iC) = numViolations / length(isi) * 100;  % percent
    end
    
    if length(ncc) > limit_spikes(1)     % implement minimum number of spikes
        [H1, ccr, lags] = acorr(ncc,wn,res, g.isvisual);
...
%         sccr2 = filtfilt(flt,1,sccr);   % high-pass filter > 1 Hz
        ...
        [thinx, pretrough, posttrough, use_pretrough, use_posttrough] = thetainx4(sccr, lags, true, false);
        ThetaIndex(iC) = thinx;   % theta index
        hold on
        plot(lags,sccr,'Color',[0 0 0])
        if g.isvisual == false
            ...
            bar(lags(lags>180&lags<200),ccr(lags>180&lags<200),'FaceColor','r','EdgeColor','r')
               ...
               %Plot cyan bars based on indicator variables
            if use_pretrough == true && use_posttrough==false
                bar(lags(pretrough), ccr(pretrough), 'FaceColor', 'c', 'EdgeColor', 'c');
            elseif  use_posttrough == true && use_pretrough==false
                bar(lags(posttrough), ccr(posttrough), 'FaceColor', 'c', 'EdgeColor', 'c');
            elseif use_pretrough == true && use_posttrough==true
                bar(lags(pretrough|posttrough),ccr(pretrough|posttrough),'FaceColor','c','EdgeColor','c');
            end
        end 
...
%         plot(lags,sccr2,'c')
    ...
    'SegmentLength','BurstIndex','Refractory','ThetaIndex','isiViolationRate')
            disp('Autosave done.')
        end
    end
...
% -------------------------------------------------------------------------
function [H1, ccr, lags] = acorr(ncc,wn,res, isvisual)
...
H1 = figure;
if isvisual
    bar(lags,ccr,'FaceColor','black','BarWidth',1)
else
...
% -------------------------------------------------------------------------
function [thinx, pretrough, posttrough, use_pretrough, use_posttrough] = thetainx4(ccr, lags, use_pretrough, use_posttrough)
...
if use_pretrough
    pretrough = lags >= 65 & lags <= 85;  % ACG first theta pre-trough
    thtrough = ccr(pretrough);
end

if use_posttrough
    posttrough = lags >= 225 & lags <= 275;   % ACG first theta post-trough
    if isempty(thtrough)
        thtrough = ccr(posttrough);
    else
        thtrough = ccr(pretrough | posttrough);
    end
end
