function stats = psth_stats_wm(spt,psth,dt,win,varargin)
...
%   Edit log: BH 8/12/12, 8/27/12, 7/15/13, MA 09/05/25
...
addParamValue(prs,'baselinewin',[-1.6 0],@(s)isnumeric(s)&isequal(length(s),2))  % baseline time window relative to the event for stat. testing, in seconds
addParamValue(prs,'testwin',[0.2 1],@(s)isnumeric(s)&isequal(length(s),2))  % test time window relative to the event for stat. testing, in seconds
...
WNb = [g.baselinewin(1)/dt+nullindex g.baselinewin(2)/dt+nullindex-1];   % baseline window; convert to indices; exlude 0
WNt = [g.testwin(1)/dt+nullindex g.testwin(2)/dt+nullindex-1];   % test window; convert to indices
WNb = round(WNb);
WNt = round(WNt);
...
if (minafter > baseline_prob) || isnan(minafter)     % putative inhibition, if it goes below baseline
    ...
    mininx = (WNt(1)-1) + find(psth(WNt(1):WNt(2))==minafter,1,'first');   % minimal firing, this avoids a bug
...
if (maxafter < baseline_prob) || isnan(maxafter)     % putative activation, if firing goes above baseline
    ...
    maxinx = (WNt(1)-1) + find(psth(WNt(1):WNt(2)) == maxafter, 1, 'first');   % maximal firing, this avoids a bug
