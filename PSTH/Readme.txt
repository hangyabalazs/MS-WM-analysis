...
figure4_panel is the main function used in ms_wm_main. 
It plots the main Figure 4A-B (spsth heatmap, averages per group & piechart) for stat based grouping.
It requires cleaned_data issued from process_ms_wm_data which has normalized spsth data & grouping/clustering matrices.
It calls example_panels to plot Fig 4C and S3C for heatmap per group, average per group,
raster & psth plots for example cells from groups. This is also used to produce clustering supplementary panels S5-6.
