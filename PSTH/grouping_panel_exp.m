function grouping_panel_exp(resdir, speaker_dir, response_categori_exp, normPSTH_exp_cl)
%GROUPING_PANEL_EXP Generates panel of plots for visualizing grouped neural responses.
%
%   GROUPING_PANEL_EXP(RES_DIR, SPEAKER_DIR, RESPONSE_CATEGORIES, NORM_PSTH) generates a
%   multi-panel figure for Exp neurons containing:
...
%       RESPONSE_CATEGORI_EXP - Cell array of response categories for each
%       neuron in Exp
%       NORMPSTH_EXP_CL      - Matrix of normalized PSTH data (neurons x time)
%
%
...
%  2025
    ...
    if isempty(response_categori_exp) || isempty(normPSTH_exp_cl)
    ...
    excellids={'NWM15_200224a_6.2'	,'NWM9_191102a_3.1','NWM15_200221a_4.1','NWM15_200220b_5.3','NWM15_200223a_1.3'};
    ...
    for i = 1:length(group_names)
        ...
        % Find indices of cells belonging to this group
         groupIndices = find(strcmp(response_categori_exp, groupLabel));
        % Collect PSTHs for this group
         psths{i} = normPSTH_exp_cl(groupIndices,:);
    end
    ...
    PSTH_plot(excellids{3}, alignevent, wn, dt, sigma, partition, bwin, twin,x_positions, y_positions, y_limits);
    
    figure(5)
    subplot('Position',[0.075,0.7673,0.12,0.165])
    heatmap(time, sorted_psths{1},30,{full_group_names{3};''},0.102, speaker_dir);
    
    % Assembly of plots in 1 figure
    figure(1)
    h = get(gcf,'Children');
    newh = copyobj(h,5); % average
    set(newh, 'Position',[0.075,0.55,0.12,0.165])
    
    figure(2)
    h = get(gcf,'Children');
    newh = copyobj(h(1),5); % scatter
    set(newh,'Position',[0.075,0.32, 0.12,0.165]);
    
    figure(4)
    h = get(gcf,'Children');
    newh = copyobj(h(1),5); % psth
    ...
    y_positions=[0; 2];
    y_limits=[0,12];
    PSTH_plot(excellids{4}, alignevent, wn, dt, sigma, partition, bwin, twin,x_positions, y_positions, y_limits);
    
    figure(5)
    subplot('Position',[0.26,0.7673,0.12,0.165])
    heatmap(time, sorted_psths{2},11,{full_group_names{4};''},0.287, speaker_dir);
    ...
    PSTH_plot(excellids{1}, alignevent, wn, dt, sigma, partition, bwin, twin,x_positions, y_positions, y_limits);
    
    figure(5)
    subplot('Position',[0.445,0.7673,0.12,0.165])
    heatmap(time, sorted_psths{3},2.3,{'Inhibited then';'Activated'},0.472,speaker_dir);
    ...
    PSTH_plot(excellids{2}, alignevent, wn, dt, sigma, partition, bwin, twin,x_positions, y_positions, y_limits);
    
    figure(5)
    subplot('Position',[0.63,0.7673,0.12,0.165])
    heatmap(time, sorted_psths{4},4.3,{'Activated then';'Inhibited'},0.657,speaker_dir);
    ...
    PSTH_plot(excellids{5}, alignevent, wn, dt, sigma, partition, bwin, twin,x_positions, y_positions, y_limits);
