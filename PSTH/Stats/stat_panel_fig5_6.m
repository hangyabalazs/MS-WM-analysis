function stat_panel_fig5_6(cleaned_data, ROC, ROCtime, pvalues, resdir, roc_analysis_type)
%STAT_PANEL_FIG5_6 Generates statistical comparison panels for neuronal firing rates.
%   STAT_PANEL_FIG5_6(cleaned_data, ROC, ROCtime, pvalues, resdir) processes and
...
%  2025
    ...
    fig1 = figure('Units', 'centimeters', 'Position', [0, 0, fig_width, fig_height]);
    
    % Define a tiled layout with 6 rows 
    tiledlayout(3, 2, 'TileSpacing', 'compact', 'Padding', 'compact');
    
    % List of axes handles from previously generated plots
    existing_axes = {axes1, axes2, axes3, axes4, axes5, axes6};
    ...
    fnm = [resdir '\'  'Stat panel - fig5.jpg'];   % save
    saveas(gcf,fnm)
    
    fnmm = [resdir '\'  'Stat panel - fig5.svg'];   % save
    saveas(gcf,fnmm)

    % Create figure with specified size in centimeters
    fig2 = figure('Units', 'centimeters', 'Position', [0, 0, fig_width, fig_height]);
    
    % Define a tiled layout with 6 rows 
    tiledlayout(3, 2, 'TileSpacing', 'compact', 'Padding', 'compact');
    
    % List of axes handles from previously generated plots
    existing_axes = {axes7, axes8, axes9, axes10};
    ...
    % Save final figure to both JPG and SVG formats
    fnm = [resdir '\'  'Stat panel - fig6' roc_analysis_type '.jpg'];   % save
    saveas(gcf,fnm)
    
    fnmm = [resdir '\'  'Stat panel - fig6' roc_analysis_type '.svg'];   % save
    saveas(gcf,fnmm)
